...
export default function LoginPage() {
  ...
  return <section className="auth-page" aria-labelledby="login-title"><section className="auth-card">
    ...
    <form className="modern-form" onSubmit={submit}>
      {stage !== "setup" && <Field label="Second Life username"><input autoFocus={stage === "username"} autoComplete="username" value={username} onChange={e => setUsername(e.target.value)} placeholder="firstname.lastname" required disabled={stage !== "username"} /></Field>}
      {stage === "password" && <Field label="Password"><input autoFocus type="password" autoComplete="current-password" value={password} onChange={e => setPassword(e.target.value)} required /></Field>}
      {stage === "otp" && <Field label="One-time code"><input autoFocus inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{8}" maxLength={8} value={code} onChange={e => setCode(e.target.value)} placeholder="8 digits" required /></Field>}
      {stage === "setup" && <><Field label="Password" help="Use at least 12 characters."><input autoFocus type="password" autoComplete="new-password" minLength={12} value={password} onChange={e => setPassword(e.target.value)} required /></Field><Field label="Confirm password"><input type="password" autoComplete="new-password" minLength={12} value={confirmation} onChange={e => setConfirmation(e.target.value)} required /></Field></>}
      <FormActions><button className="button" disabled={busy}>{busy ? "Working…" : stage === "username" ? "Continue" : stage === "password" ? "Sign in" : stage === "otp" ? "Verify account" : "Set password and sign in"}</button>
        {stage === "password" && <button type="button" className="button secondary" onClick={useOtp} disabled={busy}>Use one-time code</button>}
