...
export default async function Success({searchParams}:{searchParams:Promise<{invoice?:string}>}) {
  ...
  return <section className="panel form-shell"><div className="eyebrow">Checkout returned</div><h1 className="section-title">Payment {status.toLowerCase()}</h1><p>Returning from checkout does not activate a lease. Only the verified payment handler can do that.</p><Link className="button btn btn-primary" href={viewer?"/portal":"/login"}>{viewer?"Open resident portal":"Sign in to view payment"}</Link></section>;
}
