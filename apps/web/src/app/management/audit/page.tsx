...
export default async function Audit(){
 ...
 return <AppShell viewer={viewer} eyebrow="Management / Security" title="Audit & system health"><section className="panel worker-health"><div className="section-head"><div><h2>Service health</h2><p>Real production workers only. Heartbeats older than 30 seconds are stale.</p></div></div><div className="worker-list">{items.map(item=><article className="worker-row" key={item.id}><div><strong>{item.label}</strong><small>{item.at&&<><LocalDateTime value={item.at}/> · </>}{item.detail}</small></div><span className={`health-state ${healthState(item)}`}>{healthState(item)==="healthy"?"Healthy":"Stale / degraded"}</span></article>)}</div></section><section className="panel" aria-labelledby="security-audit-heading"><div className="section-head"><div><h2 id="security-audit-heading">Security audit</h2><p>Latest 200 security and operator events.</p></div></div><AuditTable rows={auditRows}/></section></AppShell>;
}
