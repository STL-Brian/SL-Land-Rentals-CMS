...
export default async function Reservations(){
  ...
  return <AppShell viewer={viewer} eyebrow="Management / Concierge" title="Reservations">
    <section className="panel card"><h2>Create a time-bounded reservation</h2><p>Prices remain server-owned. Reservations create no invoice, payment, or lease.</p>{listings.rowCount?<ReservationForm listings={listings.rows}/>:<p className="empty">An available listing is required.</p>}</section>
    <section className="panel card"><div className="section-head"><h2>Live reservations</h2></div>{reservations.rowCount===0?<p className="empty">No active reservations in your scope.</p>:<div className="table-scroll"><table className="table table-dark table-hover align-middle mb-0"><thead><tr><th>Rental</th><th>Customer</th><th>Created by</th><th>Expires (local time)</th><th>Notes</th><th>Action</th></tr></thead><tbody>{reservations.rows.map(row=><tr key={row.id}><td>{row.listing_name}</td><td>{row.target_name}</td><td>{row.creator_name}</td><td><LocalDateTime value={row.expires_at}/></td><td>{row.notes||"—"}</td><td><CancelReservation id={row.id}/></td></tr>)}</tbody></table></div>}</section>
  </AppShell>;
}
