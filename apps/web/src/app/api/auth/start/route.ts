...
export async function POST(req: Request) {
  try {
    ...
    const hasPassword = Boolean(found.rows[0]?.password_hash && found.rows[0].active);
    if (!hasPassword) await requestChallenge(username, ip);
    // The response intentionally does not disclose account or credential state.
    return NextResponse.json({ mode: hasPassword ? "password" : "otp" }, { headers: noStoreHeaders });
  } catch {
