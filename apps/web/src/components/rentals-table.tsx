...
export function RentalsTable({ rows }: { rows: RentalDisplayRow[] }) {
  ...
  return <>
    ...
    <div className="table-scroll"><table className="table"><thead><tr><th>Rental</th><th>Resident identity</th><th>Status</th><th>Term</th><th>Action</th></tr></thead><tbody>{visible.map((row) => <tr key={row.id}><td><strong>{row.name}</strong></td><td><strong>{row.displayName}</strong><small>{row.canonicalUsername ?? "Unlinked"}</small></td><td><span className="status">{row.status}</span></td><td>{new Date(row.startsAt).toLocaleDateString()} – {new Date(row.endsAt).toLocaleDateString()}</td><td>{row.status === "ACTIVE" ? <RentalControls id={row.id}/> : "—"}</td></tr>)}</tbody></table>{visible.length === 0 && <p className="empty">No rentals match this status.</p>}</div>
  </>;
}
