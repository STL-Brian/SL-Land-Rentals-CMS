...
export function AppShell({ viewer, title, eyebrow, children }: { viewer: Viewer; title: string; eyebrow: string; children: React.ReactNode }) {
  ...
  return <div className="app-layout">
    ...
    <div className="app-main">
      <details className="mobile-nav"><summary>Menu · {roleLabel(viewer.role)}</summary><nav>{nav.map(item => <Link key={item.href} href={item.href}>{item.label}</Link>)}<Link href="/account/password">Change password</Link></nav></details>
